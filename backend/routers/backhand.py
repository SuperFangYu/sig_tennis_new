...
from backend.services.pipeline import REPO_ROOT, build_artifact_list, run_backhand_pipeline

router = APIRouter(tags=["backhand"])
...
async def analyze(
    ...
    try:
        result = run_backhand_pipeline(dest, run_id, handedness=handedness)
    ...
    for sub in sorted(base.iterdir(), key=lambda p: p.name):
        ...
        for f in sorted(sub.iterdir()):
            ...
            if lower.endswith("_backend.csv") and "kpt" in lower:
                kind = "racket_csv"
            elif lower.endswith("_body_backhand_rtmpose.csv"):
                kind = "body_csv"
            elif lower.endswith("_plot.png") and "kpt" in lower:
                kind = "racket_chart"
            elif lower.endswith("_final_analysis.png"):
                kind = "final_chart"
            elif lower.endswith(".mp4") and "backhand" in lower.lower():
                kind = "clip"
