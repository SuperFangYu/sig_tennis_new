...
from backend.services.pipeline import REPO_ROOT, build_artifact_list, run_volley_pipeline

router = APIRouter(tags=["volley"])
...
async def analyze(
    ...
    try:
        result = run_volley_pipeline(dest, run_id, handedness=handedness)
...
def list_artifacts(run_id: str) -> ArtifactsListResponse:
    ...
    for sub in sorted(base.iterdir(), key=lambda p: p.name):
        ...
        for f in sorted(sub.iterdir()):
            ...
            lower = f.name.lower()
            name = f.name
            if "_volley_trace_chart_" in name:
                kind = "volley_trace_chart"
            elif "_volley_kinetic_chart_" in name:
                kind = "volley_kinetic_chart"
            elif "_Volley_v2_" in name and lower.endswith(".mp4"):
                kind = "clip"
            elif lower.endswith("_body_volley_rtmpose.csv"):
                kind = "body_csv"
            elif lower.endswith("_backend.csv") and "kpt" in lower:
                kind = "racket_csv"
            elif lower.endswith("_plot.png") and "kpt" in lower:
                kind = "racket_chart"
            elif lower.endswith("_final_analysis.png"):
                kind = "final_chart"
            elif lower.endswith(".mp4") and "forehand" in lower:
                kind = "clip"
            elif "_kinetic_chain_" in lower and lower.endswith(".png"):
                kind = "kinetic_chart"
            elif "_speed_cog_" in lower and lower.endswith(".png"):
                kind = "speed_cog_chart"
