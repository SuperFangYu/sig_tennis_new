...
程序用视频 YOLO 拍头速度和 QTM `Racket_top` 的 ZY 平面速度自动寻找五次动作，用五对
速度峰建立连续分段线性时间映射，再比较左右肩、肘、髋、膝 8 个角度。没有球、看不到触球都可以；
...
若发球的自动候选峰会被抛球、回拍或动作间走动干扰，可在 `serve_validation.py` 中改为手动
填写五次完整发球的视频时间段：
...
每一段都应从该次发球启动前开始，到随挥完成后结束；五段必须按时间递增、互不重叠。
程序在每段内自动选择拍头速度最高点，不要求手工填写触球时刻。最终完整动作指标严格使用
这五段边界，Qualisys 侧仍独立寻找拍头事件并执行所有关节共用的分段时间映射。
...
未填写逐次动作范围时，你不需要填写五次动作的起止时间。程序会：

1. 从视频提取拍头速度；
2. 从 QTM `Racket_top` 的 Z/Y 坐标计算拍头速度；
3. 两边各找候选速度峰，并从中匹配五个顺序一致的动作锚点；
4. 让相邻球拍锚点之间各自线性映射，组成一条连续且严格递增的分段时间函数；
...
- `alignment_qc.csv`：`status=pass`、局部斜率、全局直线诊断值、球拍有效率；
- `alignment_segments.csv`：四个相邻锚点区间的局部斜率与截距；
- `repetition_windows.csv`：五次动作的自动起止或手动填写时间是否合理；
...
任一局部斜率在 0.90–1.10 之外时不会直接判失败，但 `alignment_qc.csv` 会将
`time_scale_warning` 标为 `True`。全局直线的 slope、intercept、RMSE 和最大残差继续保存，
只用于描述时间轴的非线性程度，不再用于实际角度配对或决定分段映射是否通过。
