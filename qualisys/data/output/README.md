...
- `alignment_qc.csv` / `alignment_qc.png`：分段峰值对齐及全局直线诊断的质量检查。
- `alignment_anchors.csv`：五个视频/QTM 拍头速度锚点及全局直线诊断残差。
- `alignment_segments.csv`：相邻锚点之间四段时间映射的局部斜率与截距。
- `repetition_windows.csv`：自动识别或人工填写的五次动作起止时间。
