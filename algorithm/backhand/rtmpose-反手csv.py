...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default=r"D:\FY\mediapipe_demo\视频\个人\750025740.mp4",
        help="输入视频路径",
    )
    parser.add_argument(
        "--output-dir",
    ...
    while cap.isOpened():
        ...
        frame_data: Dict[str, float] = {
            ...
            "shoulder_turn_x_diff": float("nan"),  # 新增：双肩X差值（侧身幅度）
            "right_elbow_angle": float("nan"),
            "left_elbow_angle": float("nan"),  # 新增：左肘角度（双反发力）
            "right_knee_angle": float("nan"),
            "left_knee_angle": float("nan"),  # 新增：左膝角度（反手重心）
        ...
        if len(person_bboxes) > 0:
            ...
            if keypoints is not None and scores is not None:
                ...
                # 【更新】读取所有需要的关键点，不满足阈值则置为 NaN
                r_shoulder = kpt(HALPE_IDX["r_shoulder"]) if kpt_valid(HALPE_IDX["r_shoulder"]) else np.array(
                    [np.nan, np.nan])
                l_shoulder = kpt(HALPE_IDX["l_shoulder"]) if kpt_valid(HALPE_IDX["l_shoulder"]) else np.array(
                    [np.nan, np.nan])

                r_elbow = kpt(HALPE_IDX["r_elbow"]) if kpt_valid(HALPE_IDX["r_elbow"]) else np.array([np.nan, np.nan])
                l_elbow = kpt(HALPE_IDX["l_elbow"]) if kpt_valid(HALPE_IDX["l_elbow"]) else np.array(
                    [np.nan, np.nan])  # 新增

                r_wrist = kpt(HALPE_IDX["r_wrist"]) if kpt_valid(HALPE_IDX["r_wrist"]) else np.array([np.nan, np.nan])
                l_wrist = kpt(HALPE_IDX["l_wrist"]) if kpt_valid(HALPE_IDX["l_wrist"]) else np.array(
                    [np.nan, np.nan])  # 新增
                ...
                l_knee = kpt(HALPE_IDX["l_knee"]) if kpt_valid(HALPE_IDX["l_knee"]) else np.array(
                    [np.nan, np.nan])  # 新增

                r_ankle = kpt(HALPE_IDX["r_ankle"]) if kpt_valid(HALPE_IDX["r_ankle"]) else np.array([np.nan, np.nan])
                l_ankle = kpt(HALPE_IDX["l_ankle"]) if kpt_valid(HALPE_IDX["l_ankle"]) else np.array(
                    [np.nan, np.nan])  # 新增

                # ----------------- 特征计算 -----------------
                # 特征 1：身体中线 X（通过双肩中心）及 侧身幅度（双肩X差值）
                if kpt_valid(HALPE_IDX["r_shoulder"]) and kpt_valid(HALPE_IDX["l_shoulder"]):
                    ...
                    frame_data["shoulder_turn_x_diff"] = float(r_shoulder[0] - l_shoulder[0])

                # 特征 2：双臂肘部角度 (兼顾正反手发力特征)
                frame_data["right_elbow_angle"] = calculate_angle(r_shoulder, r_elbow, r_wrist)
                frame_data["left_elbow_angle"] = calculate_angle(l_shoulder, l_elbow, l_wrist)

                # 特征 3：双膝角度 (兼顾正反手重心与蹬地特征)
                frame_data["right_knee_angle"] = calculate_angle(r_hip, r_knee, r_ankle)
                frame_data["left_knee_angle"] = calculate_angle(l_hip, l_knee, l_ankle)
    ...
    csv_save_path = output_dir / f"{video_name}_body_backhand_rtmpose.csv"
